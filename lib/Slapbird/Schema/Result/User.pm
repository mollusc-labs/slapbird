...
__PACKAGE__->has_one(_user_pricing_plan            => 'Slapbird::Schema::Result::UserPricingPlan',           'user_id');
__PACKAGE__->has_one(_associated_user_pricing_plan => 'Slapbird::Schema::Result::AssociatedUserPricingPlan', 'user_id');
...
sub is_associated {
  my ($self) = @_;
  return $self->user_pricing_plan->user_id eq $self->user_id;
}
